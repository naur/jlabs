...
<%@ page import="com.yihaodian.scm.scs.framework.util.cache.CacheClientService" %>
...
<%@ page import="com.yihaodian.scm.scs.framework.service.RmiServerService" %>
<html>
<head>
...
<%!
    ...
    private String domFormat(Object... params) {
        return MessageFormat.format(
                ...
                params[0], StringUtils.isEmpty(params[0].toString()) ? "\r\n" + params[1] : params[1],
                loopLevel((Integer) params[2]),
                StringUtils.isEmpty(String.valueOf(params[1])) ? "style=\"font:italic bold 12px/20px arial,sans-serif;\"" : "");
    }
...
<%!
    public void rmi(Map result) {
        Map sourceMap = new HashMap();
        result.put("RMI Info", sourceMap);
        try {
            RmiServerService rmiServerService = (RmiServerService) applicationContext.getBean("rmiServerService");
            sourceMap.put("info", rmiServerService.getRmiUrl());
            sourceMap.put("rmi", true);
        } catch (Exception ex) {
            //todo log.error("get dataSource status error!", e);
            sourceMap.put("EXCEPTION", ExceptionUtils.getFullStackTrace(ex));
        }
...
<%--Cache--%>
<%!
    private void cache(Map buffer, String key) throws IntrospectionException, InvocationTargetException, IllegalAccessException {
        CacheClientService cacheClient = (CacheClientService) applicationContext.getBean("mcCache");
        buffer.put("Cache", objectToMap(cacheClient.get(key)));
    }
%>
...
<%
    ...
    try {
        ...
        for (Cookie cookie : request.getCookies()) {
            if ("user_cache_id".equals(cookie.getName())) {
                user = cacheClient.get(cookie.getValue() + "_loginUser");
            }
        }
        if (null == user) {
            buffer.put("ERROR", "请登录");
        } else if (null == params || 0 >= params.length()) {
            params = "";
            ...
            //todo 屏蔽方法调用  buffer.put("", this.execute(params, line, timeout));
